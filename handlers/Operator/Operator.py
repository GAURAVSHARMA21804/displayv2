...
from Models.Operator.Operator import operator_login, add_work, add_reading, get_task, notify_to_incharge_func,checksheet_add_logs, get_fpa_data 
...
# @TokenRequirements.token_required
def checksheet_add_logs_handler(**kwargs):
    return checksheet_add_logs(request.form)

# @Operator1.route("/operator/get_task_data",methods = ['POST'])
# @TokenRequirements.token_required
# def work_operator_data_handler(**kwargs):
#     return work_operator_get_data(request.form)


########################operator fpa dataa #######################3
@Operator1.route("/operator/fpa",methods=['POST'])
# @TokenRequirements.token_required?
def get_fpa_data_handler(**kwargs):
    return get_fpa_data(request.form)