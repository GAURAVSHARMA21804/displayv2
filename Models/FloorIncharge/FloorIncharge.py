...
from Database.models import floor_incharge_creds, Operator_creds, parts_info, processes_info, parameters_info, check_sheet_data, stations, work_assigned_to_operator, work_assigned_to_operator_logs, check_sheet, notify_to_incharge, Operator_creds
    ...
    try:
        ...
        if user is not None:
            if user.password == password:
                ...
                station_data = stations.query.filter_by(floor_no=user.floor_no).all()
                ...
                return jsonify({'Response:': 'Floor_Incharge login successfull!', 'token': f'{token}', 'employee_id':f'{user.employee_id}', 'fName':f'{user.fName}', 'lName':f'{user.lName}', 'building_no': f'{user.building_no}', 'floor_no': f'{user.floor_no}', 'total_stations': f'{len(station_ids)}', 'total_lines':f'{len(stations_dict)}'}), 200
            else:
...
def assign_task(data):
    try:
        ...
        for stationTask in data:
            ...
            if station:
                if station.end_shift_time>=current_time and station.date==current_date:
                    ...
                    reseponse[station_id] = "Please reset the all task first on this station"
                    db.session.rollback()
        ...
        return jsonify({'assigned task to': assigned_task_stations, 'running task to stations': running_task_stations, 'reset_task':reseponse}), 200
...
def refresh_data(data):
    try:
        ...
        if current_lines_data:
            ...
            return jsonify({"Message":"floor does not exists"}), 404  

            
        
        # Querying the database for records where the station_id matches any in the list
        matched_stations = db.session.query(work_assigned_to_operator).filter(work_assigned_to_operator.station_id.in_(all_stations)).all()
        # print(matched_stations[0].employee_id)
        unmatched_station_ids = [station_id for station_id in all_stations if station_id not in [station.station_id for station in matched_stations]]
        
        if matched_stations:
            employee_data_map = {}
            for station_data in matched_stations:
                if station_data.employee_id not in employee_data_map:
                    employee_data_map[station_data.employee_id] = {
                        'stations': [],
                        'process_data':[],
                        'part_data': [],
                        'data': None
                    }
                employee_data_map[station_data.employee_id]['stations'].append(station_data.station_id)
                employee_data_map[station_data.employee_id]['process_data'].append(station_data.process_no)
                employee_data_map[station_data.employee_id]['part_data'].append(station_data.part_no)


            employee_ids = list(employee_data_map.keys())
            ...
            if get_employees_data:
                for employee_data in get_employees_data:
                    ...
                    if employee_id in employee_data_map:
                        employee_data_map[employee_id]['data'] = {
                ...
                response_data = []
                for employee_id, data in employee_data_map.items():
        ...
        #                 datas[employee_data.employee_id] = {'stations': [],
        #                     'data': []}
        #             datas[employee_data.employee_id]['data'].append({
        #                 'fName': employee_data.fName,
        #                 'lName': employee_data.lName,
        #                 'skill_level': employee_data.skill_level
        #             })
        ...
        #         for station_data in matched_stations:
        #             if station_data.employee_id in employee_ids:
        #                 datas[station_data.employee_id]['stations'].append(station_data.station_id)


                return jsonify({"station_ids where no task assigend": unmatched_station_ids,"employee_data":f"{response_data}"}), 200
            else:
                return jsonify({"Message":"Employess datas does not exist."}), 404
        else:
...
#                     db.session.commit()
#                     all_stations_free = False  # Set flag to False for this station
#                 else:
#                     return jsonify({"Message": f"Station {station_id} has not assigned any task"}), 404
#             else:
#                 return jsonify({"Message": f"Station {station_id} has not assigned any task"}), 404

#         if all_stations_free:
#             return jsonify({"Message": "All stations are free now"}), 201
#         else:
#             return jsonify({"Message": "Stations are now free"}), 201  # Return a generic message for success
...
def free_stations_if_task_completed(data):
    try:
        ...
        task_running = []
        task_not_assigned = []
        stations_free = []  # List to store responses for each station

        for station_id in station_ids:
            get_station_data = work_assigned_to_operator.query.filter_by(station_id=station_id).first()
            if get_station_data:
                if get_station_data.end_shift_time >= current_time and get_station_data.date == current_date:
                    task_running.append({"station_id": station_id, "Message": f"Task is running on station {station_id}"})
                elif get_station_data.end_shift_time < current_time or get_station_data.date != current_date:
                    new_record = work_assigned_to_operator_logs(
                        employee_id=get_station_data.employee_id,
                        station_id=get_station_data.station_id,
                        part_no=get_station_data.part_no,
                        process_no=get_station_data.process_no,
                        start_shift_time=get_station_data.start_shift_time,
                    ...
                    stations_free.append({"station_id": station_id, "Message": f"Station {station_id} is free now"})
            else:
                task_not_assigned.append({"station_id": station_id, "Message": f"Station {station_id} has not assigned any task"})

        return jsonify({"task_running": task_running, "task_not_assigned": task_not_assigned, "stations_free": stations_free}), 200
